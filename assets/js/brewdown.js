...
const Brewdown = (function() {
    ...
    function renderToc(entries) {
        ...
        // data-brewdown-toc marks this as generated by ::toc::, so rebuildToc()
        // can refresh it without touching hand-written TOCs (e.g. vanity/meta/intro.md).
    ...
    function processScriptTags(root) {
        ...
        // Collected so callers can await every include; each entry settles once its
        // content is in the DOM. Fetches still run in parallel; only the waiting
        // is coordinated. See processAll(), which rebuilds ::toc:: afterwards.
        const pending = [];
            ...
            if (markdownFile) {
                ...
                pending.push(fetch(markdownFile)
                    ...
                    .then(data => {
                        ...
                        // When the include sits inside an already-rendered brewdown body,
                        // splice its blocks straight into that flow instead of leaving a
                        // wrapper div. A wrapper is a block box: it will not wrap around a
                        // floated ::toc::, it clears it, so the TOC would end up sitting
                        // above the included prose rather than beside it. Standalone
                        // includes (no brewdown parent) keep the wrapper.
    ...
    function processAll() {
        ...
        // ::toc:: is rendered synchronously, so it can only see headings written
        // directly on the page. Headings inside <script data-brewdown> files are
        // still being fetched at that point. Once they land, rebuild the TOC from
        // the finished DOM so included headings appear too.
        included.then(() => rebuildToc(root));
    }

    // Rebuild any ::toc:: on the page from the headings actually in the DOM.
    // No-op when the page has no TOC or nothing was included.
    function rebuildToc(root) {
        ...
        const entries = [];
        // Anchors from ::label:: are level 0; headings keep their own level.
        scope.querySelectorAll('h1[id], h2[id], h3[id], .toc-anchor[id]').forEach(el => {
            if (toc.contains(el)) return;
            entries.push({
                level: el.classList.contains('toc-anchor') ? 0 : parseInt(el.tagName[1]),
